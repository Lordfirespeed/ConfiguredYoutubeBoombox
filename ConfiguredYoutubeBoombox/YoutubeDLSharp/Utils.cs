...
namespace YoutubeDLSharp
{
    ...
    public static class Utils
    {
            ...
            FFmpegApi.BinaryType binary = FFmpegApi.BinaryType.FFmpeg)
        {
            ...
            var ffmpegVersion =
                JsonConvert.DeserializeObject<FFmpegApi.Root>(await (await _client.GetAsync(FFMPEG_API_URL)).Content
                    .ReadAsStringAsync());
            ...
            var downloadUrl = binary == FFmpegApi.BinaryType.FFmpeg ? ffContent.Ffmpeg : ffContent.Ffprobe;
            ...
            using (var stream = new MemoryStream(dataBytes))
            {
                using (var archive = new ZipArchive(stream, ZipArchiveMode.Read))
                {
                    if (archive.Entries.Count > 0)
                        archive.Entries[0]
                            .ExtractToFile(Path.Combine(directoryPath, archive.Entries[0].FullName), true);
                }
            }
